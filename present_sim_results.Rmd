...
```{r}
dir_experience_results <- "results_simulations/20231024_095835"
...
prognostic <- df_population_parameters[, .SD, .SDcols = grep("bY_X", names(df_population_parameters), value = TRUE)][
  , lapply(.SD, \(x) x != 0)] |> 
  rename_with(\(x) sub("bY_", "", x)) |> 
  mutate(population_parameters_num = 1:n()) |> 
  pivot_longer(-population_parameters_num, names_to = "variable") |> 
  filter(value)

  # mutate(across(everything(), ifelse(x == TRUE, "oui", "")))
TEM <- df_population_parameters[, .SD, .SDcols = grep("bY_A_X", names(df_population_parameters), value = TRUE)][
  , lapply(.SD, \(x) x != 0)] |> 
...
normal_distribution <- df_population_parameters[, .SD, .SDcols = grep("f_X", names(df_population_parameters), value = TRUE)][
  , lapply(.SD, \(x) x != 0)]

df_estimators_parameters[, c("covariate_names", "outcome_regression_model")][
  , .(covariate_names, outcome_regression_model = stringr::str_extract_all(outcome_regression_model, "X\\d"))][
    .(covariate_names, )
  ]
  mutate(outcome_regression_model =)
```
